...
<?php
    ...
    $statement = $conexion->prepare("SELECT * FROM producto WHERE categoria = 'Aro'");
    $statement->execute();
    $resultado_sala = $statement->fetchAll();
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Productos para Aro"'  ;
    } else {
        $titulo = 'Aros';
    }
    ...
    $statement = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM producto WHERE categoria = 'Aro' ORDER BY idProducto DESC LIMIT $inicio, $postPorPagina");
    ...
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Aro"' ;
    } else {
        $titulo = 'Aros';
    }
