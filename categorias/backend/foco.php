...
<?php
    ...
    $statement = $conexion->prepare("SELECT * FROM producto WHERE categoria = 'Foco'");
    $statement->execute();
    $resultado_sala = $statement->fetchAll();
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Productos para Foco"'  ;
    } else {
        $titulo = 'Focos';
    }
    ...
    $statement = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM producto WHERE categoria = 'Foco' ORDER BY idProducto DESC LIMIT $inicio, $postPorPagina");
    ...
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Foco"' ;
    } else {
        $titulo = 'Focos';
    }
    ...
    require '../frontend/foco.view.php';
