...
<?php
    ...
    $statement = $conexion->prepare("SELECT * FROM producto WHERE categoria = 'Faro'");
    $statement->execute();
    $resultado_sala = $statement->fetchAll();
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Productos para Faro"'  ;
    } else {
        $titulo = 'Faros';
    }
    ...
    $statement = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM producto WHERE categoria = 'Faro' ORDER BY idProducto DESC LIMIT $inicio, $postPorPagina");
    ...
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Faro"' ;
    } else {
        $titulo = 'Faros';
    }
    ...
    require '../frontend/faro.view.php';
