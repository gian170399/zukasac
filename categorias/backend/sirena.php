...
<?php
    ...
    $statement = $conexion->prepare("SELECT * FROM producto WHERE categoria = 'Sirena'");
    $statement->execute();
    $resultado_sala = $statement->fetchAll();
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Productos para Sirena"'  ;
    } else {
        $titulo = 'Sirenas';
    }
    ...
    $statement = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM producto WHERE categoria = 'Sirena' ORDER BY idProducto DESC LIMIT $inicio, $postPorPagina");
    ...
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Sirena"' ;
    } else {
        $titulo = 'Sirenas';
    }
    ...
    require '../frontend/sirena.view.php';
