...
<?php
    ...
    $statement = $conexion->prepare("SELECT * FROM producto WHERE categoria = 'Equipo'");
    $statement->execute();
    ...
    $statement = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM producto WHERE categoria = 'Equipo' ORDER BY idProducto DESC LIMIT $inicio, $postPorPagina");
    ...
    if (empty($resultado_sala)) {
        $titulo = 'No hay productos disponibles en "Equipo"' ;
    } else {
        $titulo = 'Equipos';
    }
    ...
    require '../frontend/equipo.view.php';
